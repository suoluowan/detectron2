...
class ChartBasedAnnotationsAccumulator(AnnotationsAccumulator):
    ...
    def pack(self, block_file=None) -> Optional[PackedChartBasedAnnotations]:
        """
        Pack data into tensors
        """
...
@dataclass
class PackedPoiseNetBasedAnnotations:
    """
    Packed annotations for chart-based model training. The following attributes
    are defined:
     - fine_segm_labels_gt (tensor [K] of `int64`): GT fine segmentation point labels
...
class PoiseNetBasedAnnotationsAccumulator(AnnotationsAccumulator):
    """
    Accumulates annotations by batches that correspond to objects detected on
    individual images. Can pack them together into single tensors.
    """
    ...
    def accumulate(self, instances_one_image: Instances, block_file=None):
        """
        Accumulate instances data for one image
    ...
    def pack(self, block_file=None) -> Optional[PackedPoiseNetBasedAnnotations]:
        """
        Pack data into tensors
        """
        if not len(self.i_gt):
        ...
        block_u_width = torch.tensor(np.array(block_file["bucket_u_width"]), dtype=torch.float32, device=u_gt.device)
        block_v_width = torch.tensor(np.array(block_file["bucket_v_width"]), dtype=torch.float32, device=v_gt.device)
        block_u_center = torch.tensor(np.array(block_file["bucket_u_center"]), dtype=torch.float32, device=u_gt.device)
        block_v_center = torch.tensor(np.array(block_file["bucket_v_center"]), dtype=torch.float32, device=v_gt.device)
        u_gt_cls, u_gt_offsets = uvToBlocks(u_gt, block_u_width, block_u_center, i_gt-1, block_u_width.shape[1])
        v_gt_cls, v_gt_offsets = uvToBlocks(v_gt, block_v_width, block_v_center, i_gt-1, block_v_width.shape[1])
        # block_u_center_gt = block_u_center.gather(0, (i_gt-1).unsqueeze(-1).repeat(1,block_u_center.shape[1]))
        # block_v_center_gt = block_v_center.gather(0, (i_gt-1).unsqueeze(-1).repeat(1,block_v_center.shape[1]))
        # block_u_width_gt = block_u_width.gather(0, (i_gt-1).unsqueeze(-1).repeat(1,block_u_width.shape[1]))
        # block_v_width_gt = block_v_width.gather(0, (i_gt-1).unsqueeze(-1).repeat(1,block_v_width.shape[1]))
        return PackedPoiseNetBasedAnnotations(
            fine_segm_labels_gt=i_gt,
            x_gt=torch.cat(self.x_gt, 0),
            y_gt=torch.cat(self.y_gt, 0),
            u_gt=u_gt,
            v_gt=v_gt,
...
def extract_packed_annotations_from_matches(
    proposals_with_targets: List[Instances], accumulator: AnnotationsAccumulator, block_file=None
) -> Any:
    for proposals_targets_per_image in proposals_with_targets:
        accumulator.accumulate(proposals_targets_per_image)
    return accumulator.pack(block_file=block_file)
