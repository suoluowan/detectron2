...
from densepose.converters import ToChartResultConverter, ToMaskConverter, ToChartResultConverterWithPoiseNet
...
def prediction_to_dict(instances, img_id, embedder, class_to_mesh_name, use_storage, block_fpath=None):
    """
    Args:
    ...
    elif isinstance(instances.pred_densepose, PoiseNetPredictorOutput):
            results_densepose = densepose_poisenet_predictions_to_dict(instances, block_fpath)
...
def densepose_poisenet_predictions_to_dict(instances, block_fpath):
    segmentations = ToMaskConverter.convert(
        instances.pred_densepose, instances.pred_boxes, instances.image_size
    )
