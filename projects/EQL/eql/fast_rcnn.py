...
class EQLFastRCNNOutputs(FastRCNNOutputs):
    ...
    def eql_loss(self):
        ...
        prob = torch.softmax(self.pred_class_logits, axis=1).detach()
        top_values, top_index = prob.topk(1194, dim=1, largest=False, sorted=True) 
        mi = self.pred_class_logits.gather(1, top_index[torch.arange(self.n_i),-1].unsqueeze(-1))
        # mi = torch.min(self.pred_class_logits, dim=1, keepdim=True)[0]
        M = torch.max(self.pred_class_logits, dim=1, keepdim=True)[0]

        correlation = torch.exp(-(self.pred_class_logits-mi)*2/(M-mi)).detach()
        # correlation = torch.exp(-(self.pred_class_logits)*3/M).detach()
        # correlation = 1. - prob
        correlation = correlation.scatter(1, top_index, 1.)
        eql_w = 1 - self.exclude_func() * self.threshold_func() * (1 - target) * correlation
