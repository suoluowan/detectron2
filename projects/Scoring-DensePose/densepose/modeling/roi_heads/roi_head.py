...
class DensePoseROIHeads(StandardROIHeads):
    ...
    def _init_densepose_head(self, cfg, input_shape):
        ...
        # scoring densepose
        self.scoring_on            = cfg.MODEL.ROI_DENSEPOSE_HEAD.SCORING_ON
        # fmt: on
        if self.use_decoder:
        ...
        # scoring densepose
        if self.scoring_on:
            self.scoring_head = build_densepose_scoring_head(cfg, self.densepose_head.n_out_channels)
            self.scoring_predictor = build_densepose_scoring_predictor(cfg, self.scoring_head.n_out_channels)
            self.scoring_losses = build_densepose_scoring_losses(cfg)
    ...
    def _forward_densepose(self, features: Dict[str, torch.Tensor], instances: List[Instances]):
        ...
        if self.training:
            ...
            if len(proposals) > 0:
                ...
                if self.scoring_on:
                    densepose_scoring_head_outputs = self.scoring_head(densepose_head_outputs)
            ...
            densepose_scoring_predictor_outputs = None
            ...
            densepose_inference(densepose_predictor_outputs, instances, densepose_scoring_predictor_outputs)
            return instances
